...
<div class="animated fadeIn">
    ...
    <form action="{{ route('admin.save_multiple_section') }}" method="post" enctype=multipart/form-data id="software_form">
        ...
        @for($i=1;$i<=$section->where('section_no', '!=', 1)->count();$i++)
            @php isset($section[$i-1]) ? ${'section'.($i+1)} = $section->where('section_no', $section[$i-1]['section_no'])->first() : '' @endphp
            ...
            <div class="collapse_sec">
                ...
                <div class="accordion custom-accordionwithicon accordion-secondary mt-0 mb-2" id="sub_section{{$i}}">
                    <div class="accordion-item">
                        ...
                        <div id="sub_sec_{{$i}}" class="accordion-collapse collapse" aria-labelledby="sub_sec_{{$i}}" data-bs-parent="#sub_section{{$i}}">
                            ...
                            <div class="accordion-body">
                                ...
                                <div class="row gy-2 mt-1">
                                    <div class="col-xxl-6 col-md-6">
                                        ...
                                        <input type="file" class="image form-control" id="file_tag_0" name="section[0][image]">
                                    </div>
                                </div>
...
<script>
    ...
    $(document).on('change', '.type', function() {
        ...
        if ($(this).val() == 2) {
            ...
            $('#file_tag_' + id).attr('type', "text").attr("value", "");
            $('#label_tag_' + id).html('Enter Link');
        } else {
            arr.splice(arr.indexOf($(this).attr('dataid')), 1);
            $('#file_tag_' + id).attr('type', "file").attr("value", "");
