    ...
    <div class="faqBox-inner">
        <div class="leftFaq-content">
            <ul class="nav nav-tabs" id="faqTab" role="tablist">
                @for($i=2; $i<=7; $i++) @php $data=$section->where('section_no', $i)->where('status', 1)->first(); @endphp
...
<script>
    ...
    function showFaqResults(e) {
        ...
        // Define search limit based on language
        let searchLimit = (currentLanguage() === 'ja') ? 1 : 3;

        if (keyword == '' || keyword.length < searchLimit) { //3
            $('.faq-results').html('');
            return false;
        }
    ...
    function currentLanguage() {
        var currentLanguageCode = "{{ app()->currentLocale() }}";
        return currentLanguageCode; // Replace with your actual variable
    }
