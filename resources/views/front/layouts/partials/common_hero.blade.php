...
<section class="other-pages-banner padding-tb-180 d-flex flex-wrap align-items-center"  @if($page && $page->slug == 'faqs' && $page->bg_img != null) style="background-image: url({{$page->bg_img}}); @else style="background-image: url({{ asset('fixifx/images/hero-bg.png') }}); @endif">
  <div class="container">
    <div class="row justify-content-end align-items-center">
      <div class="col-12 col-xl-6 col-lg-12 col-md-12 col-sm-12">
        <div class="other-banner-text hero-wrapper">
        <h1>{{ $page->{config('app.locale').'_desc'} }}</h1>
                 ...
                 @for($i = 0; $i <= count(Request::segments()); $i++) 
                    ...
                    @endphp
                      ...
                      @if($i == count(Request::segments()))  
                        @if(Request::segment($i) == 'faqs')
                            <li class="breadcrumb-item"><a href="javascript:void();">{{ $page->menu->{config('app.locale').'_name'} }}</a></li>
                        @endif
