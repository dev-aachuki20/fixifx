    ...
    <div class="container">
        ...
        <div class="header-btns">
          <ul>
            ...
            <li class="nav-item dropdown language-box">
              ...
              <a href="javascript:void();" @if(config('app.locale')=='en' ) class="nav-link active" @endif><img
                  src="{{ asset('fixifx/images/eng.png') }}"
                  alt="{{ config('app.locale') == 'ja' ? '英語版FiXi FX（フィクシー）' : 'English' }}"> ENG</a>
              @else
              <a href="javascript:void();" @if(config('app.locale')=='ja' ) class="nav-link active" @endif><img
                  src="{{ asset('fixifx/images/ja.png') }}"
                  alt="{{ config('app.locale') == 'ja' ? '日本語版FiXi FX（フィクシー）' : 'Japanese' }}"> JA</a>
              @endif
              <ul>
                <li><a href="{{ route('language.change', ['locale' => 'en']) }}" @if(config('app.locale')=='en' )
                    class="active" @endif><img src="{{ asset('fixifx/images/eng.png') }}"
                      alt="{{ config('app.locale') == 'ja' ? '英語版FiXi FX（フィクシー）' : 'English' }}">ENG</a></li>
                <li><a href="{{ route('language.change', ['locale' => 'ja']) }}" @if(config('app.locale')=='ja' )
                    class="active" @endif><img src="{{ asset('fixifx/images/ja.png') }}"
                      alt="{{ config('app.locale') == 'ja' ? '日本語版FiXi FX（フィクシー）' : 'Japanese' }}">JA</a></li>
              </ul>
            </li>
          </ul>
        </div>
      ...
      <div class="collapse navbar-collapse justify-content-between" id="navbarNavDropdown">
        <ul class="navbar-nav">
          ...
          @if(count($m->subMenu))
          <li
            class="nav-item dropdown {{($m->en_name == 'Forex Trading') ? 'megalarge_link' : ''}} {{ (in_array($route_param, $m->menuPage->pluck('slug')->toArray())) ? 'active' : '' }}">
            <a href="javascript:void();" class="nav-link dropdown-toggle" data-bs-toggle="dropdown"
              aria-expanded="false">
            ...
            <div class="mega-menu dropdown-menu">
              <a href="javascript:void();" class="back_btn">
                <svg width="6" height="10" viewBox="0 0 6 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M0.215872 5.49505L4.74173 9.79476C5.02962 10.0684 5.4964 10.0684 5.78416 9.79476C6.07195 9.52135 6.07195 9.0779 5.78416 8.80451L1.77948 4.99993L5.78405 1.19548C6.07183 0.921955 6.07183 0.478552 5.78405 0.205141C5.49626 -0.0683804 5.02951 -0.0683804 4.74161 0.205141L0.215755 4.50492C0.0718641 4.64169 0 4.82075 0 4.99991C0 5.17915 0.0720043 5.35835 0.215872 5.49505Z"
                    fill="#FFFFFF"></path>
                </svg>
                {{__('message.back') }}
              </a>
                  ...
                  <div class="col-lg-9 col-xl-9">
                    <div class="row">
                      ...
                      <dl class="col-xl-6 col-lg-8 dropdown-pf-wrap mega-menu-column">
                        <a href="javascript:void();" class="linkhead-box"><h4>{{ $submenu->{config('app.locale').'_name'} }}</h4></a>
                          ...
                          @if($submenu->id == 2)
                            <dl class="col-lg-4 pf-no-margin">
                                <dt class="m-0 mb-3 header-pf-name">
                                    ...
                                    <a href="javascript:void();"><h5 class="overviewtext">{{__('message.overview') }}</h5></a>
                                </dt>
                                <dd>
                                    <div class="mega-menu-outer">
                      ...
                      <div class="col-xl-3 col-lg-4">
                        <dl class="menu-dropdown-col mega-menu-column">
                          ...
                          <dd>
                            <div class="mega-menu-outer">
                              @foreach($submenu->menuPage->where('status', 1) as $page)
                              <div class="{{ ($route_param == $page->slug) ? 'active' : '' }} mega-menu-box"><a
                                  href="{{ route('page', [config('app.locale'), $page->slug]) }}">{{
                    ...
                    <div class="header-blog-box">
                      ...
                      <div class="blogs-disc">
                        ...
                        <div class="button-group">
                          <a href="{{ getSettingValue('member_login') }}" target="_blank"
                            class="custom-btn fill-btn-1">{{__('message.member_login', [], config('app.locale'))}}</a>
                          <a href="{{ getSettingValue('partner_login') }}" target="_blank "
                            class="custom-btn fill-btn">{{__('message.partner_login', [], config('app.locale'))}}</a>
                        </div>
                      </div>
                    </div>
                  </div>          
          ...
          <li class="nav-item {{($m->en_name == 'Home') ? 'home_menu' : ''}}">
            <a href="{{($m->en_name == 'Home') ? route('page', [config('app.locale'), 'home']) : '' }}"
              class="nav-link {{($m->en_name == 'Home') ? 'active' : '' }}">
              {{ $m->{config('app.locale').'_name'} }}</a>
          </li>
          @endif
          @endforeach
          ...
          <li class="nav-item {{ ($route_param == 'prex-blogs') ? 'active' : '' }}">
            <a href="{{ route('page', [config('app.locale'),'prex-blogs']) }}">{{__('message.fixi_blog', [],
              config('app.locale'))}}</a>
          </li>
        </ul>
        <div class="header-btns">
          <ul>
            ...
            <li class="nav-item dropdown language-box">
              ...
              @if(config('app.locale') == 'en')
              <a href="javascript:void();" class="nav-link"><img src="{{ asset('fixifx/images/eng.png') }}"
                  alt="{{ config('app.locale') == 'ja' ? '英語版FiXi FX（フィクシー）' : 'English' }}"> ENG</a>
              @else
              <a href="javascript:void();" class="nav-link"><img src="{{ asset('fixifx/images/ja.png') }}"
                  alt="{{ config('app.locale') == 'ja' ? '日本語版FiXi FX（フィクシー）' : 'Japanese' }}"> JA</a>
              @endif
              <ul>
                <li><a href="{{ route('language.change', ['locale' => 'en']) }}" @if(config('app.locale')=='en' )
